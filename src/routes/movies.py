...
async def get_movie_list(
    ...
    sort_by: str = Query(
        None, description="Sort by field: price, date, popularity, imdb, etc."
    ),
    ...
    if year:
        stmt = stmt.where(
            MovieModel.date == year
        )  # If 'date' is a date, you may need to extract year
    ...
    if price_min is not None:
        stmt = stmt.where(
            MovieModel.budget >= price_min
        )  # Or use 'price' if you have it
    if price_max is not None:
        stmt = stmt.where(
            MovieModel.budget <= price_max
        )  # Or use 'price' if you have it
    if genre:
    ...
    if director:
        stmt = stmt.join(MovieModel.directors).where(
...
async def get_movie_by_id(
    ...
    response = MovieDetailSchema.model_validate(
        movie, from_attributes=True
    ).model_dump()
...
async def like_or_dislike_movie(
    ...
    stmt = select(MovieLikeModel).where(
        MovieLikeModel.user_id == current_user.id, MovieLikeModel.movie_id == movie_id
    )
    result = await db.execute(stmt)
    ...
    if like_obj:
        ...
        like_obj = MovieLikeModel(
            user_id=current_user.id, movie_id=movie_id, is_like=data.is_like
        )
        db.add(like_obj)
    await db.commit()
    return
...
async def get_movie_like_counts(
    ...
    stmt = (
        select(MovieLikeModel.is_like, func.count())
        .where(MovieLikeModel.movie_id == movie_id)
        .group_by(MovieLikeModel.is_like)
    )
    result = await db.execute(stmt)
    rows = result.all()
    counts = {row[0]: row[1] for row in rows}
    return MovieLikeCountSchema(
        likes=counts.get(True, 0), dislikes=counts.get(False, 0)
    )
...
async def list_favorite_movies(
    ...
    stmt_fav = select(FavoriteMovieModel.movie_id).where(
        FavoriteMovieModel.user_id == current_user.id
    )
    ...
    response = MovieListResponseSchema(
        movies=movie_list,
        prev_page=(
            f"/favorites/?page={page - 1}&per_page={per_page}" if page > 1 else None
        ),
        next_page=(
...
async def rate_movie(
    ...
    if rating_obj:
        ...
        rating_obj = MovieRatingModel(
            user_id=current_user.id, movie_id=movie_id, rating=data.rating
        )
        db.add(rating_obj)
    await db.commit()
...
async def get_movie_average_rating(
    ...
    stmt = select(
        func.avg(MovieRatingModel.rating), func.count(MovieRatingModel.id)
    ).where(MovieRatingModel.movie_id == movie_id)
    result = await db.execute(stmt)
    avg, count = result.first()
    return MovieRatingAverageSchema(
        movie_id=movie_id, average_rating=avg or 0.0, ratings_count=count or 0
    )
...
async def get_my_movie_rating(
    ...
    if not rating_obj:
        raise HTTPException(
            status_code=404, detail="You have not rated this movie yet."
        )
    return rating_obj
...
async def like_or_dislike_comment(
    ...
    if like_obj:
        ...
        like_obj = MovieCommentLikeModel(
            user_id=current_user.id, comment_id=comment_id, is_like=data.is_like
        )
        db.add(like_obj)
    await db.commit()
...
async def get_comment_like_counts(
    ...
    stmt = (
        select(MovieCommentLikeModel.is_like, func.count())
        .where(MovieCommentLikeModel.comment_id == comment_id)
        .group_by(MovieCommentLikeModel.is_like)
    )
    result = await db.execute(stmt)
    rows = result.all()
    counts = {row[0]: row[1] for row in rows}
...
async def get_purchased_movies(
    ...
    stmt = (
        select(PurchasedMovieModel)
        .where(PurchasedMovieModel.user_id == current_user.id)
