...
async def update_notification(
    ...
    if notification.user_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="You can only update your own notifications."
        )
...
async def delete_notification(
    ...
    if notification.user_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="You can only delete your own notifications."
        )
...
async def admin_get_user_cart(
    ...
    if not cart:
        return {"user_id": user_id, "cart_items": [], "total_price": 0.0}
    ...
    return {"user_id": user_id, "cart_items": cart_items, "total_price": total_price}
