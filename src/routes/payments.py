...
@router.post("/", response_model=PaymentSchema, status_code=status.HTTP_201_CREATED, summary="Create payment", description="Create a payment for an order using Stripe")
...
@router.get("/", response_model=list[PaymentSchema], summary="List user payments", description="Get all payments for the current user")
async def list_user_payments(
    db: AsyncSession = Depends(get_db),
    current_user = Depends(get_current_user),
):
    stmt = select(PaymentModel).where(PaymentModel.user_id == current_user.id).order_by(PaymentModel.created_at.desc())
    result = await db.execute(stmt)
    payments = result.scalars().all()
    return payments


@router.get("/{payment_id}", response_model=PaymentSchema, summary="Get payment details", description="Get details of a specific payment for the current user")
...
@router.post("/{payment_id}/refund", response_model=PaymentSchema, summary="Refund payment", description="Refund a successful payment (admin only)")
...
@router.get("/admin/payments/", response_model=list[PaymentSchema], summary="Admin: List all payments", description="Get all payments with optional filters (admin only)")
async def admin_list_payments(
    status: Optional[PaymentStatusEnum] = Query(None, description="Filter by payment status"),
    user_id: Optional[int] = Query(None, description="Filter by user ID"),
    start_date: Optional[datetime.date] = Query(None, description="Filter payments from this date"),
    end_date: Optional[datetime.date] = Query(None, description="Filter payments until this date"),
    db: AsyncSession = Depends(get_db),
    current_admin: UserModel = Depends(get_current_admin),
):
...
@router.post("/webhook/stripe", summary="Stripe webhook", description="Handle Stripe webhook events for payment validation")
